...
def arg_parse():
    ...
    parser.add_argument(
        "--frozen_graph", type=str, help="The path of detection frozen graph is used"
    )
    parser.add_argument(
        "--box_imgs_npy_path",
        type=str,
...
def run_inference_for_images_per_image(
    graph, image_folder, np_boxes_path, score_threshold
):
    ...
    with graph.as_default():
        ...
        with tf.Session() as sess:
            path_box_lists = []
            for frame_path in tqdm(frame_lists):
...
def vis_detection_result(graph, image_path, output_image_path):
    with graph.as_default():
        ...
        with tf.Session() as sess:
            ...
            # return output_dict
            print(
                "output_dict['detection_boxes'] shape is {}".format(
                    output_dict["detection_boxes"].shape
                )
            )
            print(
                "output_dict['detection_scores'] shape is {}".format(
                    output_dict["detection_scores"].shape
                )
            )

            category_index = label_map_util.create_category_index_from_labelmap(
                PATHS.get_model_detection_label_path(), use_display_name=True
            )
...
if __name__ == "__main__":
    ...
    forzen_graph_path = (
        args.frozen_graph if args.frozen_graph else PATHS.get_model_frozen_graph_path()
    )
    graph = load_frozen_graph(forzen_graph_path)
    ...
    run_inference_for_images_per_image(
        graph, image_dataset_path, np_paths_boxes_path, 0.5
    )
